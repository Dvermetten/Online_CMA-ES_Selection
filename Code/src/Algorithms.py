...
:returns:               The statistics generated by running the algorithm
    ...
    def __init__(self, population, fitnessFunction, budget, functions, parameters, parallel=False):
        ...
        self.C_at_switch = []
        self.mean_searchpoint_over_time = []
    ...
    def setConfigurationParameters(self, functions, parameters):
        ...
        self.select = functions['select']
        # self.mutateParameters = functions['mutateParameters']
        self.parameters.weights_option = parameters['weights_option']
        self.parameters.active = parameters['active']
        self.parameters.elitist = parameters['elitist']
        self.parameters.sequential = parameters['sequential']
        self.parameters.tpa = parameters['tpa']
        self.parameters.local_restart = parameters['local_restart']
    ...
    def recordStatistics(self):
        ...
        self.sigma_over_time.append(self.parameters.sigma)
        self.fitness_over_time.extend([self.population[0].fitness] * gen_size)
        meanpoint = [y[0] for y in np.mean([ x.genotype for x in self.population], axis=0)]
        self.mean_searchpoint_over_time.append(meanpoint)
    ...
    def switchConfiguration(self, opts,finfo = None):
        if finfo is not None:
            self.load_hyperparameters(finfo)
        selector = Sel.pairwise if opts['selection'] == 'pairwise' else Sel.best
        self.C_at_switch = self.parameters.C
        ...
        self.parameters.c_mu = min(1-self.parameters.c_1, self.parameters.alpha_mu*((mu_eff - 2 + 1/mu_eff) / ((n+2)**2 + self.parameters.alpha_mu*mu_eff/2)))
        self.parameters.damps = 1 + 2*np.max([0, np.sqrt((mu_eff-1)/(n+1))-1]) + self.parameters.c_sigma
        self.seq_cutoff = self.parameters.mu_int * self.parameters.seq_cutoff



    def runSingleSplitOptimizer(self, opts, split, target, threshold=1e-8, seed = None, finfo = None):
        # print("starting run of single split optimizer")
        switched = False
        while self.used_budget < self.budget and not self.parameters.checkLocalRestartConditions(self.used_budget):
            if not switched and self.best_individual.fitness - target <= split:
                self.switchConfiguration(opts,finfo=finfo)
                switched = True
                if seed is not None:
                    np.random.seed(seed)
            elif self.best_individual.fitness - target <= threshold:
                break
            self.runOneGeneration()
            self.recordStatistics()

    def runLocalRestartSingleSplitOptimizer(self, opts, split, target, threshold=1e-8):
        """
        ...
        while self.total_used_budget < self.total_budget:
            ...
            self.runSingleSplitOptimizer(opts, split, target, threshold)
            self.total_used_budget += self.used_budget
            if target is not None:
                if self.best_individual.fitness - target <= threshold:
                    break
    ...
    def save_hyperparameters(self, finfo):
        fid = finfo[0]
        instance = finfo[1]
        runnr = finfo[2]
        filename = "/data/s1603094/hyperparameters/HyperparametersF{0}Inst{1}Runnr{2}.p".format(fid,instance,runnr)
        pickle.dump(self.parameters, open(filename,"wb"))

    def load_hyperparameters(self, finfo):
        fid = finfo[0]
        instance = finfo[1]
        runnr = finfo[2]
        filename = "/data/s1603094/hyperparameters/HyperparametersF{0}Inst{1}Runnr{2}.p".format(fid, instance, runnr)
        params_new = pickle.load(open(filename, "rb"))
        to_ignore = ['sqrt_C', 'C', 'sigma', 'p_sigma', 'offspring', 'all_offspring', 'wcm', 'wcm_old']
        for key in (params_new.__dict__.keys()):
            if key not in to_ignore:
                self.parameters.__dict__[key] = params_new.__dict__[key]


    def runOptimizer(self, target=None, threshold=1e-8, split = 10e-0, seed = None, finfo = None):
        if target is not None:
            # print(target, threshold)
            switched = False
            while self.used_budget < self.budget and self.best_individual.fitness - target > threshold and not self.parameters.checkLocalRestartConditions(
                    self.used_budget):
                if not switched and self.best_individual.fitness - target <= split:
                    self.C_at_switch = self.parameters.C
                    if finfo is not  None:
                        self.save_hyperparameters(finfo)
                    if seed is not None:
                        np.random.seed(seed)
                    switched = True
                self.runOneGeneration()
                self.recordStatistics()
            # print(self.best_individual.fitness - target, self.best_individual.fitness - target <= threshold, self.used_budget, self.total_used_budget)
    ...
    def runLocalRestartOptimizer(self, target=None, threshold=1e-8):
        """
            Run the baseAlgorithm with the given specifications using a local-restart strategy.
        """
        # print("Local restart on")
        ...
        while self.total_used_budget < self.total_budget:
            ...
            # Run the actual algorithm
            self.runOptimizer(target, threshold)
            self.total_used_budget += self.used_budget
            if target is not None:
                if self.best_individual.fitness - target <= threshold:
                    break
...
def _localRestartAlgorithm(fitnessFunction, budget, functions, parameter_opts, parallel=False):
    """
        ...
        :param parameter_opts:  Dictionary containing the all keyword options that will be used to initialize the
                                :class:`~src.Parameters.Parameters` object
        :param parallel:        Can be set to True to enable parallel evaluation. This disables sequential evaluation
...
def _customizedES(n, fitnessFunction, budget, split = None, mu=None, lambda_=None, opts=None, values=None, target=None, seed = None,finfo = None):
    if seed is not None:
        # print(seed)
        np.random.seed(seed)
    ...
    if opts is not None and opts['ipop']:
        custom_es.runLocalRestartOptimizer(target)
    else:
        custom_es.mutateParameters = custom_es.parameters.adaptCovarianceMatrix
        custom_es.runOptimizer(target, split= split,seed=seed,finfo=finfo)
    return custom_es.generation_size, custom_es.sigma_over_time, custom_es.fitness_over_time, custom_es.best_individual, np.array(custom_es.C_at_switch).flatten(), custom_es.mean_searchpoint_over_time

def _customizedSingleSplitES(n, fitnessFunction, budget, target, split, mu=None, lambda_=None, opts1=None, opts2=None, values=None, seed = None, finfo=None):
    if seed is not None:
        # print(seed)
        np.random.seed(seed)
    custom_es = CustomizedES(n, fitnessFunction, budget, mu, lambda_, opts1, values)

    if opts1 is not None and opts1['ipop']:
        custom_es.runLocalRestartSingleSplitOptimizer(opts2, split, target)
    else:
        custom_es.mutateParameters = custom_es.parameters.adaptCovarianceMatrix
        custom_es.runSingleSplitOptimizer(opts2, split=split, target=target,seed=seed, finfo=finfo)
